...
public static class UniformPoissonDiskSampler
{
    ...
    {
        Vector2 dim = lowerRight - topLeft;
    ...
    static void AddFirstPoint(ref Settings settings, ref State state)
    {
        ...
        while (!added)
        {
            ...
            if (settings.RejectionSqDistance != null && Mathf.Pow(Vector2.Distance(settings.Center, p),2f) > settings.RejectionSqDistance)
                continue;
            added = true;
    ...
    static bool AddNextPoint(Vector2 point, ref Settings settings, ref State state)
	{
        ...
        if (q.x >= settings.TopLeft.x && q.x <= settings.LowerRight.x && 
            ...
            (settings.RejectionSqDistance == null || Mathf.Pow(Vector2.Distance(settings.Center, q),2f) <= settings.RejectionSqDistance))
		{
